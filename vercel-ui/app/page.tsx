...
export default function Page() {
  ...
  const handleSubmit = async (suggestion?: string) => {
    ...
    try {
      ...
      let query = await getQueryFromLLM(JSON.stringify(inputQuery));
      const match = query.match(/```sql([\s\S]*?)```/);

      if (match) {
          // Extract the SQL content and format it to the required structure
          const sqlQuery = match[1].trim(); // Get SQL query content and trim extra spaces
          query = sqlQuery;
      }
