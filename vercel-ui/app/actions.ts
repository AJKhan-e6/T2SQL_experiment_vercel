...
export const runGenerateSQLQuery = async (query: string) => {
  ...
  try {
    ...
    data = await getQueryResultsFromConnnector(formData);
    // data = {rows: dummyData};
  } catch (e: any) {
...
export const getQueryFromLLM = async (body: any) => {
  try {
    const response = await axios.post("http://localhost:8010/", body, {
      headers: {
    ...
    console.error(e.message);
    throw new Error("Failed to retrieve query from LLM");
  }
};

export const getQueryResultsFromConnnector = async (body: FormData) => {
  try {
    const response = await axios.post("http://localhost:8011/execute-sql", body, {
      headers: {
